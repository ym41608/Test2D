...
% Dec, 2015
function [P, Q, fail] = ASIFT_RANSAC(mkeyFileName, imgFileName, in_mat, nor_mat)
  P = []; Q = []; fail = false;
  % add path
  addpath('./matcher');
  ...
  % ASIFT
  [frames1, frames2] = matcher(mkeyFileName, imgFileName, matchings);
  
  % Normalize pixel coordinates and change direction of y axis.
  frames1 = nor_mat * [frames1; ones(1, size(frames1,2))];
